...
async function getSongs(folder) {
    crrFolder = folder;
    let response = await fetch(`/SufiEcho/musics/${folder}/`);
...
const playMusic = (track, pause = false) => {
    currentSong.src = `/SufiEcho/musics/${crrFolder}/` + track;
    if (!pause) {
        currentSong.play();
        play.src = "images/pause.svg";
    }
...
async function displayAlbums() {
    let response = await fetch("/SufiEcho/musics/");
    let text = await response.text();
    let div = document.createElement("div");
    div.innerHTML = text;
    let anchors = div.getElementsByTagName("a");
    ...
    for (let e of anchors) {
        if (e.href.includes("/SufiEcho/musics")) {
            let folder = e.href.split("/").slice(-2)[0];
            let response = await fetch(`/SufiEcho/musics/${folder}/info.json`);
            ...
            cardContainer.innerHTML += `<div class="card" data-folder=${folder}>
              ...
              <img src="/SufiEcho/musics/${folder}/cover.jpg">
                  <h2>${meta.title}</h2>
